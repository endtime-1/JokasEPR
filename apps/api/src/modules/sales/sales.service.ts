...
export class SalesService {
  ...
  async options(user: AuthenticatedUser) {
    const [branches, warehouses, products, customerGroups, customers, priceLists, invoices] = await Promise.all([
      ...
      this.prisma.product.findMany({ where: { companyId: user.companyId, deletedAt: null, status: "ACTIVE", type: { not: "RAW_MATERIAL" } }, select: { id: true, sku: true, name: true, uomId: true, feedForm: true, piecesPerUnit: true, uom: { select: { symbol: true, name: true } } }, orderBy: { name: "asc" } }),
