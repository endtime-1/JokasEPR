...
const CONTACT_WINDOW_MS = 60 * 60 * 1000; // 1-hour window
const CONTACT_MAX = 5;                     // 5 contact messages per IP per hour
...
const _contactMemFallback = new Map<string, { attempts: number; windowEnd: number }>();
...
// M-BUG: this used to share StorefrontOrderRateLimitGuard's bucket and
// hardcoded "Too many orders" message — a customer who'd placed a few
// legitimate orders could then be blocked from submitting one unrelated
// contact message and shown a confusing, wrong error. Own bucket, own
// (lower) limit, own message.
@Injectable()
export class StorefrontContactRateLimitGuard implements CanActivate {
  private readonly logger = new Logger(StorefrontContactRateLimitGuard.name);

  constructor(private readonly prisma: PrismaService) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest<Request>();
    const key = `storefront:contact:${request.ip ?? "unknown"}`;
    const now = new Date();
    const windowEnd = new Date(now.getTime() + CONTACT_WINDOW_MS);
    ...
    try {
      ...
      if (existing.attempts >= CONTACT_MAX) {
        throw new HttpException("Too many messages from this IP. Try again later.", HttpStatus.TOO_MANY_REQUESTS);
      }
      ...
      if (err instanceof HttpException) throw err;
      this.logger.error("StorefrontContactRateLimit DB error — using in-memory fallback", (err as Error).message);
      return this.memCheck(key, now.getTime(), windowEnd.getTime());
    }
  }

  private memCheck(key: string, nowMs: number, windowEndMs: number): boolean {
    const entry = _contactMemFallback.get(key);
    if (!entry || entry.windowEnd <= nowMs) {
      _contactMemFallback.set(key, { attempts: 1, windowEnd: windowEndMs });
      return true;
    }
    if (entry.attempts >= CONTACT_MAX) {
      throw new HttpException("Too many messages from this IP. Try again later.", HttpStatus.TOO_MANY_REQUESTS);
    }
    entry.attempts += 1;
    return true;
