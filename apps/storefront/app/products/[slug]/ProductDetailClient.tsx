...
function ProductHeroImage({ product }: { product: PublicProduct }) {
  ...
  const src = product.imageUrl ?? PRODUCT_IMAGES[product.publicSlug];
...
export default function ProductDetailClient({ product }: { product: PublicProduct }) {
  ...
  const inCart = items.find((i) => i.product.publicSlug === product.publicSlug);
          ...
          <div>
            ...
            {product.price != null ? (
              <div className="mt-5 flex items-baseline gap-2">
                <span className="text-4xl font-black text-brandDark">GHS {product.price.toFixed(2)}</span>
                {product.unitLabel && (
              ...
              <div className="flex overflow-hidden rounded-xl border border-gray-200">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setQty(Number.isNaN(next) ? min : Math.max(min, next));
                  }}
                  aria-label="Quantity"
                  className="h-12 w-14 border-x border-gray-200 text-center text-base font-bold text-ink outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                />
                <button
