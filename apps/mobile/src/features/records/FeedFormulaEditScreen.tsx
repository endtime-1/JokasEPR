...
export function FeedFormulaEditScreen() {
  const navigation = useNavigation<any>();
  const route      = useRoute<RecordsScreenProps<"FeedFormulaEdit">["route"]>();
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      await updateFeedFormula(formulaId, {
        name: name.trim(),
        code: code.trim(),
        feedType,
      ...
      toast.show({ type: "success", message: "Formula updated successfully." });
      navigation.goBack();
    } catch {
      toast.show({ type: "error", message: "Failed to update formula. Try again." });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["bottom"]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <PageHeader icon="pencil-outline" iconColor="#d97706" title="Edit Formula" />
...
const styles = StyleSheet.create({
  ...
  submitBtn:    { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.sm, backgroundColor: "#d97706", borderRadius: 20, paddingVertical: spacing.lg },
