...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    setMobileOpen(false);
    // Dismiss any lingering data-error toast when the user navigates to a new page.
    setDataErrorToast(false);
    if (dataErrorTimer.current) { clearTimeout(dataErrorTimer.current); dataErrorTimer.current = null; }
  }, [pathname]);
  ...
  // Show a non-intrusive toast when a GET request fails with a non-transient error.
  // We do NOT auto-remount pages here — remounting blanks visible data and makes things
  // look worse. Page-level Retry buttons handle targeted re-fetching; the toast's
  // Refresh button does a full reload as the nuclear option.
  useEffect(() => {
    function onDataError() {
      if (dataErrorTimer.current) clearTimeout(dataErrorTimer.current);
      setDataErrorToast(true);
      // Auto-dismiss after 30 s so the toast doesn't linger forever.
      dataErrorTimer.current = setTimeout(() => setDataErrorToast(false), 30000);
    }
  ...
  useEffect(() => {
    apiFetch<ApiEnvelope<{ count: number }>>("/alerts/unread-count", { quiet: true })
  ...
  const dataErrorToastEl = dataErrorToast && !apiDownBanner ? (
    <div className="fixed bottom-4 right-4 z-[9998] flex items-center gap-3 rounded-xl bg-red-600 px-4 py-3 text-sm font-medium text-white shadow-xl">
      <span>Some data failed to load. Use the Retry button on the page, or click Refresh.</span>
      <button
        className="shrink-0 rounded border border-white/40 px-3 py-1 text-xs font-semibold hover:bg-white/20"
        onClick={() => window.location.reload()}
      >
